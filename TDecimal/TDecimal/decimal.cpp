...
TDecimal::TDecimal(unsigned value) : size(0)//конструктор для типа unsigned
{
	*this = value;
}

TDecimal::TDecimal(const string& value) : size(0)//конструктор для типа string
{
	*this = value;
}

TDecimal &TDecimal::operator = (unsigned value)//оператор присваивания для типа unsigned
{
	size = 0;
...
TDecimal &TDecimal::operator =  (const string& value)//оператор присваивания для типа string
{
	size = value.size();
...
TDecimal TDecimal::operator + (const TDecimal& dec)//оператор сложения 
{
	TDecimal a, b;
	a = *this;
	b = dec;
...
TDecimal TDecimal::operator - (const TDecimal& dec)//оператор вычитания
{
	TDecimal a;
	int c = 0;
	try
...
std::ostream& operator << (ostream& os, const TDecimal& digit)//оператор вывода
{
